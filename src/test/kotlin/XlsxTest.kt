...
class XlsxTest {
    ...
    fun mapXlsxVerticalSingle() {
        ...
        val readCommand =
            XlsxReadCommand(area = "A2:B4", repeat = false, sheet = 1, spreadsheetMLPackage = spreadsheetMLPackage)
        val result = main.process(readCommand)
    ...
    fun mapXlsxVerticalMultiple() {
        ...
        val readCommand =
            XlsxReadCommand(area = "A2:B5", repeat = true, sheet = 2, spreadsheetMLPackage = spreadsheetMLPackage)
        val result = main.process(readCommand)
        Assert.assertEquals(3, result.size)
        Assert.assertEquals("HP Color LaserJet CP5520", result[0].getContent().name)
        println(result)
